...
# you can pass in your own namespace of args to use smoothie as a library
def main(args: Namespace = None):
    ...
    add_arg('-trim', '-t',
            help=' Trim out the parts you don\'t want to render,'
                 ' as either a timecode (hh:mm:ss) or frames (start-end)',
            nargs=1,
    ...
    for idx, file in enumerate(args.input):
        ...
        args.input[idx] = os.path.abspath(expand_path(file))
    ...
    if args.output:
        ...
        if args.output == 'NULL':
            ...
            args.output = os.path.abspath(expand_path(args.output))

            outf = os.path.dirname(args.output) \
                if os.path.isfile(args.output) else args.output
